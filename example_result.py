...
class Generate_角色1(ourlib.Sprite):  # DEVcomment: each Sprite has its own Generator Class.
  ...
  async def onGreenFlag():  # run when user click the green flag.     # DEVcomment: there's detailed comments in the final result.
    loop = asyncio.get_event_loop()
    loop.create_task(self.onGreenFlag_1())
    loop.create_task(self.onGreenFlag_2())
    ...

  async def onGreenFlag_1():
    self.moveToXY(10, 10)  # move to (10, 10).     # DEVcomment: self.moveToXY is defined in 'ourlib.Sprite'.
    ...
  
  async def onGreenFlag_2():
